...
const TEAMS: Record<string, string[]> = {
  ...
  nba: ['ATL','BOS','BKN','CHA','CHI','CLE','DAL','DEN','DET','GS','HOU','IND','LAC','LAL','MEM','MIA','MIL','MIN','NO','NY','OKC','ORL','PHI','PHX','POR','SAC','SA','TOR','UTA','WAS'],
  mlb: ['ARI','ATL','BAL','BOS','CHC','CHW','CIN','CLE','COL','DET','HOU','KC','LAA','LAD','MIA','MIL','MIN','NYM','NYY','OAK','PHI','PIT','SD','SEA','SF','STL','TB','TEX','TOR','WSH'],
  nhl: ['ANA','ARI','BOS','BUF','CAR','CBJ','CGY','CHI','COL','DAL','DET','EDM','FLA','LA','MIN','MTL','NJ','NSH','NYI','NYR','OTT','PHI','PIT','SEA','SJ','STL','TB','TOR','VAN','VGK','WPG','WSH'],
};
...
export const favteamCmd: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (!league && !team) {
      ...
      if (favs.length === 0) {
        await interaction.reply({ content: 'You have no favorite teams set. Use `/favteam <league> <team>` to set one.', ephemeral: true });
        return;
      }
    ...
    if (!league || !team) {
      await interaction.reply({ content: 'Please provide both a league and team. Example: `/favteam nfl PHI`', ephemeral: true });
      return;
    }
    ...
    await interaction.reply({ content: `${emoji} Set your favorite ${league.toUpperCase()} team to **${team}**!`, ephemeral: true });
  },
};
